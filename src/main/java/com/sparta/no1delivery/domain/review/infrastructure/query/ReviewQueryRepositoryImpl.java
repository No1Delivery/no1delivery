...
import static com.sparta.no1delivery.domain.review.domain.QReview.review;
...
public class ReviewQueryRepositoryImpl implements ReviewQueryRepository {
    ...
    @Override
    public Page<Review> findAllByUser(Long userId, ReviewQueryDto.Search search, Pageable pageable) {
        BooleanBuilder builder = createBuilder(search);
    ...
    private BooleanBuilder createBuilder(ReviewQueryDto.Search search) {
        ...
        if (StringUtils.hasText(search.getKeyword())) {
            ReviewQueryDto.SearchOption option = Objects.requireNonNullElse(search.getOption(), ReviewQueryDto.SearchOption.ALL); // option이 null이면 기본값 ALL
            builder.and(getSeachCondition(option, search.getKeyword()));
        }
        ...
        if (search.getStoreIds() != null && !search.getOrderIds().isEmpty()) {
    ...
    private BooleanExpression getSeachCondition(ReviewQueryDto.SearchOption option, String keyword) {
